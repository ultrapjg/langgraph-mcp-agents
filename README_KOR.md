...
> **참고**: 백엔드 실행 전에 `uv pip install -r requirements.txt`(또는 `pip install -r requirements.txt`) 명령으로 의존성을 설치해야 합니다. Python 3.12가 필요합니다.
...
```
Streamlit 앱은 기본적으로 `BACKEND_URL` 환경 변수를 통해 `http://localhost:8000` 주소의 백엔드와 통신합니다. 백엔드가 다른 호스트나 포트에서 실행된다면 이 변수를 변경하세요.
